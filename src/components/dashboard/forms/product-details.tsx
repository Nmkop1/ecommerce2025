      ...
      const response = await upsertProduct(
        {
          ...
          variantImage: values.images[0].url,
    ...
    <AlertDialog>
      <Card className="w-full">
        <CardHeader>
          <CardTitle>
            {isNewVariantPage
              ? `Dodaj nowy wariant do ${data.name}`
              : "Utwórz nowy produkt"}
          </CardTitle>
          <CardDescription>
            {data?.productId && data.variantId
              ? `Zaktualizuj informacje o produkcie ${data?.name}.`
              : " Utwórzmy produkt. Możesz edytować produkt później ze strony produktu."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
